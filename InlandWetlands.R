...
#### Parameters and Paths ####
  # Calculations requires the following parameters and their associated uncertainty:
      #Emissions from soil draining
          ...
          # EF_CH4_drained: CH4 emissions factor from soil draining (kg CH4/ha/yr)
          # EF_N2O_drained: N2O emissions factor from soil draining (kg N2O-N/ha/yr)
...
####GHG Calculations####
    ...
    for(j in c(1:length(scen))){
      ...
      for(m in c(1:nx)){
        ####Land conversion- forest biomass removal and fires####
            if(interv_luc=="yes" && s=="business_as_usual"){
              ...
              if(temp$fire_used=="True"){
                ...
                if(land_type_init == "Peatland Extraction"){
        ...
        ####Soil emissions####
          if(temp$land_use_type!="Rewetting for wetland restoration"){
    ...
    #Extract mean and sd from each subAOI and store in results df 
        resdf <- matrix(0, length(AOIs), 2+nyears*12)
        resdf[, 1] <- AOIs
        ...
        for(a in c(1:length(AOIs))){
          ...
          for(z in c(1:(nyears*3))){
            resdf[a, z+2] <- mean(temp_res[,z+3])
            resdf[a, z+2+(nyears*3)] <- sd(temp_res[,z+3])
          }
        }
        for(k in c(1:(nyears*6))){
          resdf[, 2+(nyears*6)+k] <- resdf[,2]*resdf[,2+k]
        }
    ...
    # convert to json output
        ...
        for(r in c(1:nrow(resdf))){
          aoi_tib <-tibble(
            ...
            sdCO2ha = list(unname(unlist(resdf[r,1+nyears*3+c(1:nyears)]))),
            CO2_tyr = list(unname(unlist(resdf[r,1+nyears*6+c(1:nyears)]))),
            sdCO2 = list(unname(unlist(resdf[r,1+nyears*9+c(1:nyears)]))),
            N2O_thayr = list(unname(unlist(resdf[r,1+nyears+c(1:nyears)]))),
            sdN2Oha = list(unname(unlist(resdf[r,1+nyears*4+c(1:nyears)]))),
            N2O_tyr = list(unname(unlist(resdf[r,1+nyears*7+c(1:nyears)]))),
            sdN2O = list(unname(unlist(resdf[r,1+nyears*10+c(1:nyears)]))),
            CH4_thayr = list(unname(unlist(resdf[r,1+nyears*2+c(1:nyears)]))),
            sdCH4ha  = list(unname(unlist(resdf[r,1+nyears*5+c(1:nyears)]))),
            CH4_tyr = list(unname(unlist(resdf[r,1+nyears*8+c(1:nyears)]))),
            sdCH4  = list(unname(unlist(resdf[r,1+nyears*11+c(1:nyears)])))
          )
